...
apply_patches() {
  if [[ ${#PATCHES[@]:-0} -eq 0 ]]; then info "Sem patches"; return 0; fi
...
run_hook() {
  ...
  if [[ -x ${hook} ]]; then spinner_run "Hook $1" "${hook}" "${WORKDIR}" "${ROOTFS}" "${PKGDEST}"; fi
}
...
configure_step() {
  ...
  if [[ -x "${RECIPE_DIR}/hooks/configure" ]]; then
    ...
    case "${BUILD_SYSTEM}" in
      autotools)
        mkdir -p "${BUILDDIR}"; pushd "${BUILDDIR}" >/dev/null || die "Falha em BUILDDIR"
      ...
      meson)
        ...
        ;;
      make)
        mkdir -p "${BUILDDIR}"; pushd "${WORKDIR}" >/dev/null || die "Falha em WORKDIR"
...
install_step() {
  ...
  mkdir -p "${PKGDEST}"
  local use_fr; use_fr=$(choose_fakeroot)
  local dir=${BUILDDIR:-${WORKDIR}}
  ...
  if [[ -s $(manifest_path) ]]; then
    local count; count=$(wc -l <"$(manifest_path)")
    success "Instalado ${BOLD}${NAME}${RESET} ${DIM}($(manifest_path): ${count} arquivos)${RESET}"
  else
    success "Instalado ${BOLD}${NAME}${RESET}"
  fi
}
...
init_toolchain() {
  ...
  mkdir -p "${TOOLS}" "${ROOTFS}" "${ROOTFS}"/{usr,lib,bin,include}
...
############################################
create_recipe() {
  # create_recipe <nome> [categoria]
  local name=${1:?Informe <nome>}
  local cat=${2:-${DEFAULT_CATEGORY}}
  local dir="${REPO}/${cat}"
...
${BOLD}Uso:${RESET}
  ...
  $0 fetch   <caminho/receita>                  # baixar fontes
  $0 extract <caminho/receita>                  # extrair fontes
  $0 patch   <caminho/receita>                  # aplicar patches
  $0 configure <caminho/receita>                # configurar
  $0 build <caminho/receita>                    # compilar (sem instalar)
  $0 install <caminho/receita>                  # construir e instalar
  $0 remove <caminho/receita>                   # desinstalar usando manifest
  $0 info <caminho/receita>                     # informações do pacote
  $0 toolchain init                             # inicializar toolchain básico
  $0 clean                                      # limpar WORK e BUILD

${BOLD}Layout de receitas:${RESET}
  ${REPO}/{base,extras,x11,desktop}/<nome>.recipe
...
main() {
  ...
  case "${cmd}" in
    help|-h|--help) usage ;;
    init)      setup_dirs; check_deps; success "Estrutura pronta" ;;
    new)       setup_dirs; create_recipe "${2:-}" "${3:-}" ;;
    fetch)     load_recipe "${2:?Informe receita}"; setup_dirs; check_deps; fetch_sources ;;
